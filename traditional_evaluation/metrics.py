...
class Metrics:
    ...
    def compute_bleu(self):
        ...
        for ref, gen in self.pairs:
            ...
            back_bleu_score = sentence_bleu(
                gen_tokens, ref_tokens, smoothing_function=smoothing_function
            )
        ...
        avg_backwards_bleu = (
            sum(backwards_bleu_scores) / len(backwards_bleu_scores)
            if backwards_bleu_scores
            else 0
        )
        ...
        final_scores = {"bleu": avg_bleu, "backwards_bleu": avg_backwards_bleu}
    ...
    def compute_bert_score(self):
        ...
        final_scores = {"precision": avg_precision, "recall": avg_recall, "f1": avg_f1}
    ...
    def compute_jaccard_distance(self):
        ...
        # pointwise scores
        pointwise_scores = {"jaccard_distance": jaccard_scores}

        final_scores = {"jaccard_distance": avg_jaccard}
    ...
    def diversity_scores(self, config, verbose=False):
        ...
        def get_homogenization_score(homo_type, verbose=False):
            ...
            homogenization_score_gt = (
                homogenization_score(gt, homo_type, verbose=False)
                if self.compute_gt
                else "None"
            )
            homogenization_score_gen = homogenization_score(
                gen, homo_type, verbose=False
            )
        ...
        if config["ngram"]:
            ...
            ngram_diversity_score_gt = (
                ngram_diversity_score(gt, 4) if self.compute_gt else "None"
            )
    ...
    def length_scores(self):
        ...
        # pointwise scores
        pointwise_scores = {"lengths_gt": lengths_gt, "lengths_gen": lengths_gen}
    ...
    def flesch_reading_scores(self):
        ...
        if self.compute_gt:
            flesch_reading_scores_gt, avg_flesch_reading_score_gt = (
                compute_flesch_reading_scores(gt)
            )
        else:
            flesch_reading_scores_gt, avg_flesch_reading_score_gt = "None", "None"
        flesch_reading_scores_gen, avg_flesch_reading_score_gen = (
            compute_flesch_reading_scores(gen)
        )
    ...
    def get_luar_embeddings(self, text_series, luar_tokenizer, luar_embedder):
        ...
        for i in range(num_batches):
            ...
            tokenized_texts = luar_tokenizer(
                texts,
                max_length=512,
                padding="max_length",
                truncation=True,
            ...
            tokenized_texts["input_ids"] = tokenized_texts["input_ids"].reshape(
                actual_batch_size, 1, -1
            )
            tokenized_texts["attention_mask"] = tokenized_texts[
                "attention_mask"
            ].reshape(actual_batch_size, 1, -1)
    ...
    def luar_score(self):
        ...
        luar_embedder = AutoModel.from_pretrained(
            "rrivera1849/LUAR-CRUD", trust_remote_code=True
        ).to(self.device)
        ...
        for ref_emb, gen_emb in zip(ref_embeddings, gen_embeddings):
            cosine_sim = cosine_similarity(ref_emb.unsqueeze(0), gen_emb.unsqueeze(0))[
                0
            ][0]
        ...
        # pointwise scores
        pointwise_scores = {"luar_cosine_similarities": cosine_similarities}

        # final scores
        final_scores = {"avg_luar_cosine_similarity": avg_cosine_similarity}
    ...
    def author_attribution(self, verbose=False):
        ...
        for author, stories in self.author_profile.items():
            embeddings = self.get_luar_embeddings(
                stories, luar_tokenizer, luar_embedder
            )
        ...
        test_embeddings = self.get_luar_embeddings(
            author_stories, luar_tokenizer, luar_embedder
        )
        # if verbose:
        ...
        if self.compute_gt:
            ...
            gt_embeddings = self.get_luar_embeddings(
                refs, luar_tokenizer, luar_embedder
            )
        ...
        authorwise_avg_scores = {
            author: sum(scores) / len(scores)
            for author, scores in authorwise_scores.items()
        }

        # compute average of average cosine similarity
        author_attr_cosine_similarity = sum(authorwise_avg_scores.values()) / len(
            authorwise_avg_scores
        )
        ...
        if self.compute_gt:
            ...
            for idx, author_idx in enumerate(author_index):
                gt_authorwise_scores[author_idx].append(
                    gt_cosine_similarities[idx, author_idx]
                )

            # compute average cosine similarity for each author
            gt_authorwise_avg_scores = {
                author: sum(scores) / len(scores)
                for author, scores in gt_authorwise_scores.items()
            }

            # compute average of average cosine similarity
            gt_author_attr_cosine_similarity = sum(
                gt_authorwise_avg_scores.values()
            ) / len(gt_authorwise_avg_scores)
        else:
