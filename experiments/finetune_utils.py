...
def clear_evidence(writing_sheet):
    ...
    for cctr, cat in enumerate(categories):
        ...
        writing_sheet_categories[cat] = remove_evidence(writing_sheet_temp)
...
def load_data(split="profile", writing_sheet=False, writing_summary=False):
    ...
    if os.path.exists(f"../datasets/finetune_data{writing_sheet_suffix}/{split}.csv"):
        # load the data
        finetune_df = pd.read_csv(
            f"../datasets/finetune_data{writing_sheet_suffix}/{split}.csv"
        )
        if split == "profile":
            val_finetune_df = pd.read_csv(
                f"../datasets/finetune_data{writing_sheet_suffix}/val.csv"
            )
        else:
...
def get_base_model(
    base_model_name: str, quantize: bool
    ...
    print(
        f"Loading model {'with' if quantize else 'without'} quanitization: {base_model_name}"
    )
...
def get_model(
    ...
    if test and model_name:
        ...
        if pt_model_name:
            print(
                f"Initializing trainable model from pre-trained LoRA adapters: {pt_model_name}"
            )
            model = PeftModel.from_pretrained(
                model,
                get_checkpoint_path(pt_model_name),
                is_trainable=True,
...
def get_prompt(sample, tokenizer: PreTrainedTokenizer = None, args=None):
    ...
    if not args.writing_sheet and not args.writing_summary:
        ...
        if args.test:
            ...
            context = (
                f"Writing Sheet: {writing_sheet}\tWriting Prompt: {writing_prompt}"
            )
                ...
                self.data.append(
                    {
                        ...
                        "wp": sample["writing_prompt"],
                        "prompt": prompt,
                        "label": sample["story"] + tokenizer.eos_token,
                    }
                )
            else:
                excluded += 1
...
class SFTExpandedCollator:
    ...
    def __call__(self, batch):
        ...
        label_mask = torch.arange(input_ids.shape[1]).repeat(
            input_ids.shape[0], 1
...
def test(args, test_df):
    ...
    model = get_model(
        base_model, True, model_name=args.model_name, quantize=args.quantize
    )

    collator = TestingCollator(tokenizer)
    test_dataset = SFTExpandedDataset(test_df, tokenizer, args)
    data_loader = DataLoader(
        test_dataset, args.test_batch_size, collate_fn=collator, shuffle=False
    )
    ...
    for batch in tqdm(data_loader):
        ...
        pred_stories = tokenizer.batch_decode(
            outputs[:, batch["input_ids"].shape[1] :], skip_special_tokens=True
        )
        results.extend(
            [
                {**sample, "pred_story": pred_story}
                for sample, pred_story in zip(batch["meta_data"], pred_stories)
            ]
        )
