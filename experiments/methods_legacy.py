...
class StoryGenMethods:
        ...
        for fctr, file in tqdm(
            ...
            for ictr, example in tqdm(
                ...
                prompt = construct_story_prompt(
                    example, source_constraints, few_shot_examples
                )
    ...
    def perform_vanilla(
        self, source="Reddit", few_shot=False, debug=False, is_profile=False
    ):
        """
    ...
    def schema_user_profile(self, source="Reddit", debug=False, few_shot_top_k=1):
        ...
        # system instructions
        system_instructions_combine_path = (
            f"{self.user_profile_instructions_dir}/system_prompts/combine.txt"
        )
        # user instructions
        user_instructions_combine_path = (
            f"{self.user_profile_instructions_dir}/user_prompts/combine.txt"
        )
        ...
        for file in tqdm(
            ...
            for ectr, example in tqdm(
                ...
                if ectr == 0:
                    ...
                    try:
                        prev_prompt_dict = {
                            "previous_combined_user_sheet": extract_writing_sheet(
                                user_profile_response[ectr - 1], key=prev_key
                            )
                        }
                        current_prompt_dict = {
                            "current_writing_prompt": example["writing_prompt"],
                            "current_user_sheet": extract_writing_sheet(
                                user_sheet_response[ectr], key="user_writing_sheet"
                            ),
                        }
        ...
        system_instructions_story_rules_path = (
            f"{self.user_profile_instructions_dir}/system_prompts/rules.txt"
        )
        # user instructions
        user_instructions_story_rules_path = (
            f"{self.user_profile_instructions_dir}/user_prompts/rules.txt"
        )
        ...
        for fctr, file in tqdm(
            ...
            try:
                ...
                user_profile = extract_writing_sheet(
                    user_profile_response[-1], key="combined_user_sheet"
                )
            except Exception as e:
                continue
    ...
    def rule_generator(self, source="Reddit", is_profile=False, debug=False):
        ...
        def construct_rule_extractor_prompt(
            ...
            user_instruction_dict = {
                "writing_prompt": writing_prompt,
                "ground_truth_story": profile_story,
    ...
    def personalized_rule_generator(
        self, source="Reddit", debug=False, few_shot_top_k=1
    ):
        """
        ...
        for fctr, file in tqdm(
            ...
            for ectr, example in tqdm(
                ...
                _, profile_indices = self.get_few_shot_examples(
                    profile_data, example, source=source, top_k=3
                )
    ...
    if extract_rules:
        ...
        if choice == 1:
            ...
            story_gen_methods.perform_vanilla(
                source=source, few_shot=few_shot, debug=debug, is_profile=is_profile
            )
        elif choice == 2:
            # User Profile (No Schema)
            ...
            story_gen_methods.schema_user_profile(
                source=source, debug=debug, few_shot_top_k=few_shot_top_k
            )
        elif choice == 4:
            # Rule Generator
