...
class StoryGenMethods:
    def __init__(self):
        ...
        self.generate_story_user_profile_instructions_dir = (
            "instructions/generate_story_user_profile"
        )
    ...
    def get_few_shot_examples(self, profile_data, example, source="Reddit", top_k=1):
        ...
        for pctr, pindex in enumerate(profile_indices):
            ...
            user_instruction = self.construct_user_instruction(
                profile_data[pindex], source
            )
            few_shot_examples[pctr] = {
                "User": user_instruction,
                "Assistant": profile_data[pindex]["story"],
            }

        return few_shot_examples, profile_indices

    def perform_story_generation(
        self,
        source="Reddit",
        few_shot=False,
        story_output_dir=None,
        source_constraints_dir=None,
        persona_dir=None,
        system_instructions="",
        debug=False,
        is_profile=False,
        ...
        def construct_story_prompt(
            example, source_constraints, few_shot_examples=None, persona=None
        ):
            """
            ...
            prompt = construct_prompt_message(
                system_intructions_new,
                user_instruction,
                user_constraints,
                few_shot_examples,
        ...
        if llama:
            story_output_dir = story_output_dir.replace(
                self.output_dir, self.llama_output_dir
            )

        if llama70:
            story_output_dir = story_output_dir.replace(
                self.output_dir, self.llama70_output_dir
            )
        ...
        for fctr, file in tqdm(
            enumerate(os.listdir(test_dir)),
            ...
            if persona_dir is not None:
                ...
                persona = re.search(
                    rf"<persona_prompt>(.*?)</persona_prompt>",
                    persona_data_raw,
                    re.DOTALL,
                ).group(1)
                if not persona:
            ...
            for ictr, example in tqdm(
                ...
                # few_shot
                if few_shot:
                    few_shot_examples, _ = self.get_few_shot_examples(
                        profile_data, example, source=source, top_k=few_shot_top_k
                    )
                else:
                ...
                if iterate_over_source_constraints:
                    source_constraints_path = (
                        f'{source_constraints_dir}/{file.split(".")[0]}.txt'
                    )
                    ...
                    if os.path.exists(source_constraints_path):
                        ...
                        try:
                            ...
                            source_constraints = re.search(
                                r"<story_rules>(.*?)</story_rules>",
                                source_constraints_raw,
                                re.DOTALL,
                            ).group(1)
                ...
                prompt = construct_story_prompt(
                    example, source_constraints, few_shot_examples, persona
                )
                ...
                try:
                    if llama:
                        ...
                        response = prompt_llama_router(
                            prompt, max_tokens=4096, temperature=0.7, top_p=0.95
                        )
                    else:
                    ...
                    response = None
                results.append(
                    {"writing_prompt": example["writing_prompt"], "story": response}
                )
    ...
    def perform_vanilla(
        ...
        system_instructions_path = (
            f"{self.vanilla_prompt_instructions_dir}/system_prompts/{source}.txt"
        )
        ...
        source_constraints_path = (
            f"{self.vanilla_prompt_instructions_dir}/user_prompts/{source}.txt"
        )
        ...
        self.perform_story_generation(
            source=source,
            few_shot=few_shot,
            story_output_dir=output_dir,
            source_constraints_dir=source_constraints_path,
    ...
    def no_schema_user_profile(
        self, source="Reddit", debug=False, llama=False, port_choice=1, llama70=False
    ):
        """
        ...
        def construct_user_profile_prompt(examples):
            ...
            # construct OpenAI prompt
            prompt = construct_prompt_message(
                system_instructions, user_instruction, user_constraints
            )
            return prompt
        ...
        # sytem instructions
        system_instructions_path = (
            f"{self.user_profile_instructions_dir}/system_prompts/no_schema.txt"
        )
        # user instructions
        user_instructions_path = (
            f"{self.user_profile_instructions_dir}/user_prompts/no_schema.txt"
        )
        ...
        for file in tqdm(
            ...
            output_file_path = os.path.join(
                user_profile_output_dir, file.split(".")[0] + ".txt"
            )
            ...
            if os.path.exists(output_file_path):
                ...
                # call the OpenAI model
                user_profile_response = prompt_openai(
        ...
        self.perform_story_generation(
            ...
            source_constraints_dir=user_profile_output_dir,
            system_instructions=system_instructions,
            debug=debug,
            llama=llama,
            port_choice=port_choice,
    ...
    def schema_user_profile(
        ...
        def construct_user_sheet_prompt(author_history, category_value=None):
            ...
            # construct OpenAI prompt
            prompt = construct_prompt_message(
                system_instructions_sheet, user_instruction, user_constraints_sheet
            )
            return prompt
        ...
        def construct_story_rules_prompt(writing_prompt, user_profile):
            ...
            """
            # construct the user instruction
            user_instruction_dict = {
                "Author Writing Sheet": user_profile,
            ...
            # construct OpenAI prompt
            prompt = construct_prompt_message(
                system_instructions_story_rules,
                user_instruction,
                user_constraints_story_rules,
            )
            return prompt
        ...
        def construct_persona_prompt(user_profile):
            ...
            # construct OpenAI prompt
            prompt = construct_prompt_message(
                system_instructions_persona, user_instruction, user_constraints_persona
            )
            return prompt
        ...
        # sytem instructions
        system_instructions_sheet_path = (
            f"{self.user_profile_instructions_dir}/system_prompts/schema.txt"
        )
        # user instructions
        user_instructions_sheet_path = (
            f"{self.user_profile_instructions_dir}/user_prompts/schema.txt"
        )
        # categories
        categories_path = (
            f"{self.user_profile_instructions_dir}/user_prompts/compare_categories.json"
        )
        ...
        # iterate through each file in the profile directory
        for fctr, file in tqdm(
            enumerate(os.listdir(profile_dir)),
        ...
        # system instructions
        system_instructions_story_rules_path = (
            f"{self.rule_extractor_instructions_dir}/system_prompts/delta_schema.txt"
        )
        # user instructions
        user_instructions_story_rules_path = (
            f"{self.rule_extractor_instructions_dir}/user_prompts/delta_schema.txt"
        )
        ...
        for fctr, file in tqdm(
            ...
            try:
                ...
                user_profile = extract_writing_sheet(
                    user_profile_response[0], key="writing_style"
                )
            ...
            for ectr, example in enumerate(test_data):
                ...
                # construct the prompt
                prompt = construct_story_rules_prompt(
                    example["writing_prompt"], user_profile
                )
                # call the OpenAI model
                try:
                    response = prompt_openai(prompt, max_tokens=4096, temperature=0.0)
        ...
        if persona:
            ...
            # system instructions
            system_instructions_persona_path = (
                f"{self.rule_extractor_instructions_dir}/system_prompts/persona.txt"
            )
            # user instructions
            user_instructions_persona_path = (
                f"{self.rule_extractor_instructions_dir}/user_prompts/persona.txt"
            )
            ...
            for fctr, file in tqdm(
                ...
                try:
                    # open user profile response
                    user_profile_response_path = os.path.join(
                        user_sheets_output_dir, file
                    )
                    with open(user_profile_response_path, "r") as f:
                        user_profile_response = json.load(f)
        ...
        self.perform_story_generation(
            ...
            few_shot=True,
            story_output_dir=story_output_dir,
            source_constraints_dir=story_rules_output_dir,
    ...
    def rule_generator(self, source="Reddit", is_profile=False, debug=False):
        ...
        def construct_rule_extractor_prompt(
            system_instructions,
            writing_prompt,
            profile_story,
            base_story,
            ...
            user_instruction_dict = {
                "Writing Prompt": writing_prompt,
                "Author Written Story": profile_story,
        ...
        # system instructions
        with open(
            f"{self.rule_extractor_instructions_dir}/system_prompts/delta_rules.txt",
            "r",
        ) as f:
            system_instructions = f.read()
        ...
        # user instructions
        with open(
            f"{self.rule_extractor_instructions_dir}/user_prompts/delta_rules.txt", "r"
        ) as f:
            user_constraints = f.read()

        # iterate through each file in the profile directory
        for fctr, file in tqdm(
            enumerate(os.listdir(consider_dir)),
            ...
            for ectr, example in tqdm(
                enumerate(human_data), desc=f"Processing {file}", total=len(human_data)
            ):
                # check if the example already exists in the story rules response
                if ectr < len(story_rules_response):
                    continue
                ...
                # construct the prompt
                prompt = construct_rule_extractor_prompt(
                    system_instructions,
                    writing_prompt,
                    profile_story,
                    base_story,
    ...
    def personalized_rule_generator(
        self,
        source="Reddit",
        debug=False,
        few_shot_top_k=1,
        ...
        def construct_personalized_story_rules_prompt(
            system_instructions, writing_prompt, few_shot_examples, user_constraints
        ):
            """
        ...
        # system instructions
        with open(
            f"{self.rule_extractor_instructions_dir}/system_prompts/rule_generator.txt",
            "r",
        ) as f:
            system_instructions = f.read()
        ...
        # user instructions
        with open(
            f"{self.rule_extractor_instructions_dir}/user_prompts/rule_generator.txt",
            "r",
        ) as f:
            user_constraints = f.read()
        ...
        for fctr, file in tqdm(
            ...
            try:
                ...
                profile_story_rules_path = os.path.join(
                    profile_story_rules_input_dir, file
                )
            ...
            # iterate through each example in the test data
            for ectr, example in tqdm(
                enumerate(test_data), desc=f"Processing {file}", total=len(test_data)
            ):
                # check if the example already exists in the story rules response
                if ectr < len(story_rules_response):
                    continue
                ...
                for pctr, pindex in enumerate(profile_indices):
                    ...
                    few_shot_examples[pctr] = {
                        "User": f"Writing Prompt: {profile_data[pindex]['writing_prompt']}",
                        "Assistant": f"Story Rules: {profile_rules}",
                    }

                # construct the prompt
                prompt = construct_personalized_story_rules_prompt(
                    system_instructions,
                    example["writing_prompt"],
                    few_shot_examples,
                    user_constraints,
                )
                # call the OpenAI model
    ...
    def delta_user_profile(
        ...
        def construct_user_sheet_prompt(
            system_instructions,
            writing_prompt,
            profile_story,
            base_story,
        ...
        def construct_user_profile_prompt(prev_prompt_dict, current_prompt_dict):
            ...
            user_instruction = ""
            input_prompt_dict = {
                "Previous_Information": prev_prompt_dict,
                "Current_Information": current_prompt_dict,
            }
            user_instruction += f"{json.dumps(input_prompt_dict, indent=4)}\n\n"

            # construct OpenAI prompt
            prompt = construct_prompt_message(
                system_instructions_combine, user_instruction, user_constraints_combine
            )
            return prompt
        ...
        def add_references(user_sheet_response, reference_num="1"):
            ...
            """
            # Extract text between the tags <writing_style></writing_style>
            user_sheet = re.search(
                r"<writing_style>(.*?)</writing_style>", user_sheet_response, re.DOTALL
            ).group(1)
            ...
            for i, category in enumerate(categories):
                ...
                end_idx = (
                    user_sheet.find(categories[i + 1], start_idx)
                    if i + 1 < len(categories)
                    else len(user_sheet)
                )
                content = user_sheet[
                    start_idx + len(category) : end_idx
                ].strip()  # Extract content after category
        ...
        # system instructions
        with open(
            f"{self.user_profile_instructions_dir}/system_prompts/delta_schema.txt", "r"
        ) as f:
            system_instructions = f.read()

        # user instructions
        with open(
            f"{self.user_profile_instructions_dir}/user_prompts/delta_schema.txt", "r"
        ) as f:
            user_constraints = f.read()
        ...
        for fctr, file in tqdm(
            ...
            for ectr, example in tqdm(
                ...
                profile_story = example["story"]
                # construct the prompt
                prompt = construct_user_sheet_prompt(
                    system_instructions,
                    writing_prompt,
                    profile_story,
                    base_story,
        ...
        for fctr, file in tqdm(
            ...
            for ectr, example in tqdm(
                ...
                if ectr == 0:
                    user_profile_response.append(
                        add_references(user_sheet_response[ectr])
                    )
                else:
                    # construct the prompt
                    if ectr == 1:
                    ...
                    try:
                        prev_prompt_dict = {
                            "previous_combined_author_sheet": extract_writing_sheet(
                                user_profile_response[ectr - 1], key=prev_key
                            )
                        }
                        current_prompt_dict = {
                            "Story Identifier": ectr + 1,
                            "current_writing_prompt": example["writing_prompt"],
                            "current_author_sheet": add_references(
                                user_sheet_response[ectr], reference_num=str(ectr + 1)
                            ),
                        }

                        # construct the prompt
                        prompt = construct_user_profile_prompt(
                            prev_prompt_dict, current_prompt_dict
                        )
                        # call the OpenAI model
                        response = prompt_openai(
                            prompt, max_tokens=4096, temperature=0.0
        ...
        for fctr, file in tqdm(
            ...
            try:
                ...
                if len(user_profile_response) == 1:
                    user_profile = extract_writing_sheet(
                        user_profile_response[-1], key="writing_style"
                    )
                else:
                    user_profile = extract_writing_sheet(
...
def parse_args():
    ...
    # source
    parser.add_argument(
        "--source", type=str, default="Reddit", help="Source of the data"
    )
    ...
    # is_profile
    parser.add_argument(
        "--is_profile", action="store_true", help="generate on profile data"
    )
    # extract rules
    ...
    parser.add_argument(
        "--llama",
        action="store_true",
        help="Llama 8B for Generation via sglang local server",
    )
    # llama 70B mode
    parser.add_argument(
        "--llama70", action="store_true", help="Llama 70B for Generation via OpenRouter"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Choice of the port: 1. 30000, 2. 50000",
    )
...
def main():
    ...
    if persona and choice != 3 and choice != 5:
        raise ValueError(
            "Persona mode is only available for User Profile (Schema) and User Profile (Delta)"
        )
    ...
    if extract_rules:
        # extract delta rules
        story_gen_methods.rule_generator(
            source=source, is_profile=is_profile, debug=debug
        )
    else:
        if choice == 1:
            # perform Vanilla story generation
            story_gen_methods.perform_vanilla(
                source=source,
                few_shot=few_shot,
                debug=debug,
                is_profile=is_profile,
            ...
            # User Profile (No Schema)
            story_gen_methods.no_schema_user_profile(
                source=source,
                debug=debug,
                llama=llama,
                port_choice=port_choice,
            ...
            # User Profile (Schema)
            story_gen_methods.schema_user_profile(
                source=source,
                debug=debug,
                few_shot_top_k=few_shot_top_k,
                persona=persona,
            ...
            # Rule Generator
            story_gen_methods.personalized_rule_generator(
                source=source,
                debug=debug,
                few_shot_top_k=few_shot_top_k,
                llama=llama,
            ...
            # User Profile (Delta)
            story_gen_methods.delta_user_profile(
                source=source,
                debug=debug,
                few_shot_top_k=few_shot_top_k,
                persona=persona,
            ...
            # oracle generator
            story_gen_methods.perform_oracle(
                source=source,
                debug=debug,
                llama=llama,
                port_choice=port_choice,
