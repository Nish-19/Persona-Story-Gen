...
def main():
    ...
    for rctr, result_raw in enumerate(results_list):
        ...
        source, category, order, run_name = process_custom_id(
            custom_id, list(sizes.keys())
        )
        ...
        if len(source_wise_results[source][run_name]) == 0:
            source_wise_results[source][run_name] = {
                category: {"1": output, 2: f"A: {order}"}
            }
        else:
            source_wise_results[source][run_name][category] = {
                "1": output,
                2: f"A: {order}",
            }
