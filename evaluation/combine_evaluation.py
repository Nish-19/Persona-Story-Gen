...
def get_overall_winner(data):
    ...
    assert all(
        len(judge) == len(next(iter(judge_values))) for judge in judge_values
    ), "Judges have different number of labels"
...
def main():
    ...
    # source names
    source_names = ["AO3", "Reddit", "Storium", "narrativemagazine", "newyorker"]
    ...
    for run in run_names:
        ...
        for source in source_names:
            ...
            for judge, judge_val in judge_dict.items():
                # winner list path
                winner_list_path = (
                    f"{stats_dir}/{run}/{judge}/{source}/winner_score.json"
                )
                ...
                winner_dict_stats[run][source][judge_val] = {
                    key: winner_dict_stats[run][source][judge_val][key]
                    for key in ["expts", "vanilla", "Tie"]
                    if key in winner_dict_stats[run][source][judge_val]
                }
            # get overall winner
            try:
                winner_dict[run][source]["combined"] = get_overall_winner(
                    winner_dict[run][source]
                )
                # store stats
                winner_dict_stats[run][source]["combined"] = Counter(
                    winner_dict[run][source]["combined"]
                )
    ...
    combined_prometheus = [
        item for sublist in all_prometheus.values() for item in sublist
    ]
